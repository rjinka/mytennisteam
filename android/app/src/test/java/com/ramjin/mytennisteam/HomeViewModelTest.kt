...
import com.ramjin.mytennisteam.data.model.HomeData
...
class HomeViewModelTest {
    ...
    fun `createCourt success`() = runTest {
        ...
        whenever(apiService.getCourts(any(), any())).thenReturn(listOf(
            Court(
                "c1",
                "New Court",
                "1"
    ...
    fun `updateCourt success`() = runTest {
        ...
        val group = Group("1", "Group 1", emptyList())
        val homeData =
            HomeData(group, emptyList(), emptyList(), listOf(Court("c1", "Court 1", "1")))
        (viewModel.homeData as MutableLiveData).value = homeData
        whenever(apiService.updateCourt(any(), any(), any())).thenReturn(
            Court(
                "c1",
                "Updated Court",
                "1"
    ...
    fun `generateRotation success`() = runTest {
        ...
        whenever(apiService.getPlayers(any(), any())).thenReturn(emptyList())
        whenever(apiService.getCourts(any(), any())).thenReturn(emptyList())
        whenever(apiService.getRotationButtonState(any(), any())).thenReturn(
            RotationButtonState(
                true,
                "",
                false
